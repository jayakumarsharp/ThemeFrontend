...
import { Grid, Card, CardContent, CardHeader, Typography, List, ListItem, ListItemText, Container, Box } from '@mui/material';
...
const QuoteDetailed = () => {
  ...
  const generateListItem = ({ label, subtext, text }) => {
    ...
    return (
      <ListItem>
        <ListItemText
          primary={<span className="listLabel">{label} {subtext && <span className="listSubtext">({subtext})</span>}</span>}
  ...
  return (
    <Container>
      {quoteSummary ? (
        <>
          <Typography variant="h1">{quoteSummary?.price?.shortName ?? "-NAME NOT FOUND-"} <span className="fs-5">({quoteSummary?.price?.symbol})</span></Typography>
          <Typography variant="h6"><Star symbol={quoteSummary?.price?.symbol} /></Typography>
            ...
            <Grid item md={7}>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <Typography variant="body2"><span className="summaryLabel">Industry</span> {quoteSummary?.summaryProfile?.industry ?? "Not found..."}</Typography>
                  <Typography variant="body2"><span className="summaryLabel">Sector</span> {quoteSummary?.summaryProfile?.sector ?? "Not found..."}</Typography>
                </CardContent>
              </Card>
            </Grid>
            <Grid item md={5}>
